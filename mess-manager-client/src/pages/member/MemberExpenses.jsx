...
const Expenses = () => {
    const { expenses, members, refreshData, globalMonth } = useData();
    const { user } = useAuth();
        ...
        <div className="space-y-6 pb-32">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">{monthLabel} Mess Transparency</p>
                </div>
                <div className="flex items-center gap-3">
                    <div className="hidden sm:block px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl shadow-sm">
                        <p className="text-[9px] font-extrabold text-emerald-700 dark:text-emerald-400 uppercase tracking-widest text-center mb-0.5 whitespace-nowrap">My Personal Total</p>
                        <p className="text-lg font-extrabold text-emerald-600 dark:text-emerald-400 text-center leading-none">₹{myApprovedTotal}</p>
                    </div>
                    <button
                        onClick={async () => {
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white font-extrabold text-xs uppercase tracking-wider rounded-xl transition-all shadow-md shadow-indigo-500/20 active:scale-95"
                    >
                ...
                {categoryStats.filter(s => s.key === 'deposit' || s.key === 'admin_balance' || s.total > 0).map((stat) => {
                    ...
                    return (
                        <Card
                            key={stat.key}
                            className={cn(
                                "p-4 sm:p-5 transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 group cursor-pointer relative overflow-hidden",
                                activeCategory === stat.key
                                    ? `ring-2 ring-indigo-500 bg-white dark:bg-slate-900 shadow-md`
                                    : "bg-white/80 dark:bg-slate-900/80 border-slate-200/80 dark:border-white/5"
                            )}
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-2">
                                        <div className={cn("p-2 rounded-xl border backdrop-blur-md shadow-sm", stat.bg, stat.border, stat.color)}>
                                            <Icon size={16} />
                                        </div>
            ...
            {/* Member Summary Card */}
            <Card className="p-6 bg-white/80 dark:bg-slate-900/80 border-slate-200/80 dark:border-white/5 overflow-hidden relative shadow-sm">
                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-4">
                        <TrendingUp className="text-indigo-600 dark:text-indigo-400" size={16} />
                        <p className="text-indigo-600 dark:text-indigo-400 font-extrabold text-[10px] uppercase tracking-wider">Live Analysis</p>
                    </div>
                    ...
                    {selectedMember !== 'all' ? (
                        <div className="space-y-5">
                            <div className="flex flex-wrap items-end justify-between gap-4">
                                <div>
                                    <p className="text-slate-400 font-extrabold text-[10px] uppercase tracking-widest mb-1">Active Contributor</p>
                                ...
                                <div className="px-4 py-2 bg-indigo-500/10 rounded-2xl border border-indigo-500/20">
                                    <p className="text-[9px] font-extrabold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider text-center mb-0.5">Total Monthly</p>
            ...
            {/* Filters */}
            <Card
                className="bg-slate-50/90 dark:bg-slate-950/80 backdrop-blur-md border border-slate-200/80 dark:border-white/5 mb-6 shadow-sm rounded-2xl"
                innerClassName="px-4 sm:px-5 py-3.5 flex flex-wrap items-center gap-4"
            >
                <div className="flex items-center gap-2">
                    <Filter size={15} className="text-indigo-600 dark:text-indigo-400" />
                    <span className="text-xs font-extrabold text-slate-800 dark:text-slate-200 uppercase tracking-wider">Filters:</span>
                </div>

                {/* Category Tabs */}
                <div className="flex items-center overflow-x-auto custom-scrollbar gap-1 p-1 bg-slate-200/60 dark:bg-slate-800/40 rounded-xl">
                    <button
                        onClick={() => setActiveCategory('all')}
                        className={cn(
                            "flex-shrink-0 px-3.5 py-1.5 rounded-lg text-xs font-extrabold transition-all duration-200 uppercase tracking-wider",
                            activeCategory === 'all'
                                ? "bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm"
                                : "text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-200 hover:bg-white/50"
                        )}
                    >
                        All
                    </button>
                ...
                <div className="flex-1 min-w-[180px] sm:ml-auto">
                    <select
                        ...
                        className="w-full px-3.5 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl text-xs font-extrabold tracking-wider uppercase text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-indigo-500/20 outline-none transition-all cursor-pointer"
                    >
            ...
            <Card className="overflow-hidden p-0 border border-slate-200/80 dark:border-white/5 shadow-sm">
                <div className="overflow-auto max-h-[600px] custom-scrollbar">
                    <table className="w-full text-left relative border-collapse">
                        <thead className="sticky top-0 z-10 bg-slate-50/95 dark:bg-slate-900/95 backdrop-blur-md shadow-sm text-slate-600 dark:text-slate-400 font-extrabold uppercase tracking-wider text-[11px] border-b border-slate-200/80 dark:border-white/5">
                            <tr>
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                            {filteredExpenses.map(expense => (
                                <tr key={expense._id || expense.id} className="hover:bg-slate-50/80 dark:hover:bg-white/5 transition-colors group">
                                    <td className="p-4 text-slate-500 dark:text-slate-400 text-xs font-bold">{expense.date}</td>
                                    <td className="p-4 font-extrabold text-slate-900 dark:text-slate-100 text-sm">{expense.description || expense.title}</td>
                                    <td className="p-4">
                                        <span className={cn(
                                            "text-[10px] px-2.5 py-1 rounded-xl font-extrabold uppercase tracking-wider border flex items-center gap-1.5 w-fit",
                                            expense.category === 'market' && "bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border-indigo-500/20",
                                            expense.category === 'spices' && "bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20",
                                            expense.category === 'rice' && "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20",
                                            expense.category === 'deposit' && "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20",
                                            expense.category === 'wifi' && "bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-500/20",
                                            expense.category === 'gas' && "bg-rose-500/10 text-rose-700 dark:text-rose-400 border-rose-500/20",
                                            expense.category === 'electric' && "bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20",
                                            expense.category === 'others' && "bg-slate-500/10 text-slate-700 dark:text-slate-400 border-slate-500/20"
                                        )}>
                                    ...
                                    <td className="p-4 font-extrabold text-slate-900 dark:text-slate-50 text-sm">₹{expense.amount}</td>
