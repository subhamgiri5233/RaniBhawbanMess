...
const MemberCard = memo(({ m, offM, dRate, dHead, dMinLimit, setEditingMember, mCount, bills, categoryProviders }) => {
    ...
    return (
        <div className={cn(
            ...
            "bg-slate-50 dark:bg-black/30 border-slate-100 dark:border-white/10 hover:border-primary-500/40 shadow-sm hover:shadow-xl hover:shadow-primary-500/5"
        )}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                ...
                <div className={cn(
                    ...
                    rem > 0 ? "bg-rose-50 dark:bg-black/40 border-rose-100 dark:border-rose-500/10" :
                        rem < 0 ? "bg-emerald-50 dark:bg-black/40 border-emerald-100 dark:border-emerald-500/10" :
                            "bg-slate-50 dark:bg-black/40 border-slate-100 dark:border-white/5"
                )}>
                    <div className={cn(
                        ...
                        return cats.map(cat => {
                            ...
                            return (
                                <div key={cat.k} className={cn(
                                    ...
                                    val > 0
                                        ? "bg-white dark:bg-black/40 border-slate-200 dark:border-white/10 shadow-sm"
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-2">
                        {[
                            { label: 'Cleared Accounts', val: counts.clear, icon: CheckCircle2, color: 'emerald', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', text: 'text-emerald-600' },
                            { label: 'Pending Payments', val: counts.pending, icon: Clock, color: 'rose', bg: 'bg-rose-500/10', border: 'border-rose-500/20', text: 'text-rose-600' },
                            { label: 'Partial Payments', val: counts.partial, icon: Activity, color: 'amber', bg: 'bg-amber-500/10', border: 'border-amber-500/20', text: 'text-amber-600' }
                        ].map((card, i) => (
                            <div key={i} className={cn("p-4 rounded-2xl border flex items-center justify-between shadow-sm", card.bg, card.border)}>
                                <div className="flex items-center gap-3">
                                    <div className={cn("p-2 rounded-xl text-white shadow-lg", `bg-${card.color}-500 shadow-${card.color}-500/20`)}>
                                        <card.icon size={18} />
                                    </div>
                                    <div>
                                        <p className="text-[9px] font-black uppercase text-slate-500 dark:text-slate-400 tracking-widest">{card.label}</p>
                                        <p className={cn("text-xl font-black leading-none mt-1", card.text)}>{card.val}</p>
                                    </div>
                                </div>
                    ...
                    <Card className="p-0 overflow-hidden border-emerald-200/50 dark:border-white/10 bg-white dark:bg-slate-900 shadow-xl">
                        <div className="p-4 sm:p-5 border-b border-slate-50 dark:border-white/5 bg-slate-50 dark:bg-black/30 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div><h2 className="text-[11px] sm:text-xs font-black text-slate-700 dark:text-emerald-200 uppercase tracking-tight">Financial Audit Trace</h2></div>
                            </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-0 divide-x divide-slate-100 dark:divide-white/5">
                            {[
                    ...
                    <Card className="p-0 overflow-hidden border-slate-200 dark:border-white/10 bg-white dark:bg-slate-900 mt-6 shadow-xl">
                        <div className="p-4 sm:p-5 border-b border-slate-50 dark:border-white/5 bg-slate-50 dark:bg-black/30 flex items-center justify-between">
                            <div className="flex items-center gap-3"><div className="p-2 bg-slate-100 dark:bg-white/5 text-slate-500 rounded-lg"><Home size={18} /></div><h2 className="text-[11px] sm:text-xs font-black text-slate-700 dark:text-slate-100 uppercase tracking-tight">Fixed Operational Bills</h2></div>
                        </div>
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 p-4 sm:p-6">
                            {['gas', 'wifi', 'electric', 'paper', 'didi', 'houseRent', 'spices', 'others'].map(key => {
                                ...
                                return (
                                    <div key={key} className="bg-slate-50 dark:bg-black/40 p-4 rounded-2xl border border-slate-200 dark:border-white/10">
                                        ...
                                        <div className="text-lg font-black tracking-tight text-slate-900 dark:text-slate-100">₹{val > 0 ? Number(val).toLocaleString() : '00'}</div>
                                    </div>
                                );
                            })}
                        </div>
                        <div className="p-4 bg-slate-50 dark:bg-black/40 border-t border-slate-100 dark:border-white/10 flex items-center justify-center gap-8">
                            ...
                            <div className="w-px h-6 bg-slate-200" /><div className="flex items-center gap-2"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Shared:</span><span className="text-base font-black text-indigo-600">₹{Math.round(stats.shared)}</span></div>
                        </div>
                    </Card>
