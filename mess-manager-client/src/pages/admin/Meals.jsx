...
import { Calendar, Plus, Trash2, Users, TrendingUp, Sparkles, ArrowRight, UtensilsCrossed, Info, Trash, X, SlidersHorizontal, Hash, ChevronDown, ChevronUp, Check } from 'lucide-react';
...
const Meals = () => {
    ...
    // --- Admin Meal Override State ---
    const [showOverridePanel, setShowOverridePanel] = useState(false);
    const [overrideInputs, setOverrideInputs] = useState({}); // { memberId: string }
    const [overrideSaving, setOverrideSaving] = useState({}); // { memberId: bool }

    // Sync override inputs when mealOverrides changes
    useEffect(() => {
        const init = {};
        members.forEach(m => {
            const mId = m._id || m.id;
            init[mId] = mealOverrides[mId] !== undefined ? String(mealOverrides[mId]) : '';
        });
        setOverrideInputs(init);
    }, [mealOverrides, members]);

    const handleSaveOverride = async (memberId) => {
        const val = overrideInputs[memberId];
        const num = val === '' ? null : Number(val);
        if (num !== null && (isNaN(num) || num < 0)) {
            alert('Please enter a valid non-negative number or leave blank to clear the override');
            return;
        }
        setOverrideSaving(prev => ({ ...prev, [memberId]: true }));
        const result = await setMealOverride(memberId, globalMonth, num);
        if (!result.success) alert('Failed to save: ' + result.error);
        setOverrideSaving(prev => ({ ...prev, [memberId]: false }));
    };

    const handleClearOverride = async (memberId) => {
        setOverrideInputs(prev => ({ ...prev, [memberId]: '' }));
        setOverrideSaving(prev => ({ ...prev, [memberId]: true }));
        const result = await setMealOverride(memberId, globalMonth, null);
        if (!result.success) alert('Failed to clear: ' + result.error);
        setOverrideSaving(prev => ({ ...prev, [memberId]: false }));
    };
    ...
    return (
        <div className="space-y-6 sm:space-y-8 pb-12">
            ...
            {/* Admin Meal Override Section */}
            {isAdmin && (
                <div className="space-y-4 pt-8 border-t border-slate-200/80 dark:border-white/5">
                    <button
                        onClick={() => setShowOverridePanel(p => !p)}
                        className="w-full flex items-center justify-between px-5 py-4 bg-amber-500/5 border border-amber-500/20 rounded-2xl hover:bg-amber-500/10 transition-all group"
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                                <p className="font-extrabold text-amber-900 dark:text-amber-300 text-sm uppercase tracking-wide">
                                    Set Member Total Meals
                                </p>
                                <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold mt-0.5">
                                    Override the total meal count for any member for {globalMonth} (admin only)
                                </p>
                            </div>
                        </div>
                        <div className="text-amber-600 dark:text-amber-400 transition-transform">
                            {showOverridePanel ? <ChevronUp size={18} /> : <ChevronDown size={18} />}
                        </div>
                    </button>

                    {showOverridePanel && (
                        <Card className="p-0 overflow-hidden shadow-sm border border-amber-500/20">
                            <div className="p-4 border-b border-slate-100 dark:border-white/5 bg-amber-500/5">
                                <div className="flex items-center gap-2">
                                    <Hash size={14} className="text-amber-600 dark:text-amber-400" />
                                    <h3 className="text-xs font-extrabold text-amber-700 dark:text-amber-300 uppercase tracking-wider">Meal Count Override — {globalMonth}</h3>
                                </div>
                                <p className="text-[10px] text-slate-500 dark:text-slate-400 font-bold mt-1">
                                    Enter a total meal count to override the actual daily records. Leave blank to use real count. Saves are immediate.
                                </p>
                            </div>
                            <div className="divide-y divide-slate-100 dark:divide-white/5">
                                {members.map(member => {
                                    const mId = member._id || member.id;
                                    const hasOverride = mealOverrides[mId] !== undefined;
                                    const actualCount = (meals || []).filter(m =>
                                        m.memberId === mId && !m.isGuest
                                    ).length;
                                    const isSaving = overrideSaving[mId];
                                    return (
                                        <div key={mId} className="flex items-center justify-between gap-4 px-5 py-3.5 hover:bg-slate-50/80 dark:hover:bg-white/5 transition-all">
                                            <div className="flex items-center gap-3 min-w-0 flex-1">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="font-extrabold text-slate-800 dark:text-slate-100 text-sm truncate">{member.name}</p>
                                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">
                                                        Actual: {actualCount} meals
                                                        {hasOverride && (
                                                            <span className="ml-2 px-1.5 py-0.5 bg-amber-500/10 text-amber-600 dark:text-amber-400 rounded-md">
                                                                Override: {mealOverrides[mId]}
                                                            </span>
                                                        )}
                                                    </p>
                                                </div>
                                            ...
                                            <div className="flex items-center gap-2 shrink-0">
                                                <div className="relative">
                                                    ...
                                                    <input
                                                        type="number"
                                                        min="0"
                                                        value={overrideInputs[mId] ?? ''}
                                                        onChange={e => setOverrideInputs(prev => ({ ...prev, [mId]: e.target.value }))}
                                                        placeholder={String(actualCount)}
                                                        className="w-24 pl-7 pr-3 py-2 text-xs font-extrabold bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-white/10 rounded-xl focus:ring-2 focus:ring-amber-500 outline-none text-slate-900 dark:text-slate-100"
                                                    />
                                                </div>
                                                <button
                                                    onClick={() => handleSaveOverride(mId)}
                                                    disabled={isSaving}
                                                    title="Save override"
                                                    className="w-8 h-8 flex items-center justify-center bg-amber-500/10 hover:bg-amber-600 hover:text-white text-amber-600 dark:text-amber-400 rounded-xl border border-amber-500/20 transition-all active:scale-90 disabled:opacity-50"
                                                >
                                                    {isSaving
                                                ...
                                                {hasOverride && (
                                                    <button
                                                        onClick={() => handleClearOverride(mId)}
                                                        disabled={isSaving}
                                                        title="Clear override"
                                                        className="w-8 h-8 flex items-center justify-center bg-rose-500/10 hover:bg-rose-600 hover:text-white text-rose-500 rounded-xl border border-rose-500/20 transition-all active:scale-90 disabled:opacity-50"
                                                    >
                                                        <X size={14} />
                                                    </button>
                                                )}
